...
class Application:
    def __init__(self):
        ...
        self.panel.place(x=135, y=40, width=640, height=640)
        self.panel2 = tk.Label(self.root)  # initialize image panel
        self.panel2.place(x=135, y=40, width=640 * 0.6, height=640 * 0.6)
        ...
        self.btcall.config(text="Clear", font=("Courier", 14))
        ...
        self.predictions = []
        self.threshold = 0.5
        self.bounding_box = {'top': 50, 'left': 980, 'width': 1100, 'height': 1100}
        self.sct = mss()
        self.video_loop()
    ...
    def video_loop(self):
        ...
        if True:
            ...
            predicts = self.hs.suggest(self.word.lower())
            if (len(predicts) > 0):
                self.bt1.config(text=predicts[3], font=("Courier", 20))
            else:
                self.bt1.config(text="")
            if (len(predicts) > 1):
                self.bt2.config(text=predicts[4], font=("Courier", 20))
            else:
                self.bt2.config(text="")
            if (len(predicts) > 2):
                self.bt3.config(text=predicts[5], font=("Courier", 20))
            else:
                self.bt3.config(text="")
            if (len(predicts) > 3):
                self.bt4.config(text=predicts[6], font=("Courier", 20))
            else:
                self.bt4.config(text="")
            if (len(predicts) > 4):
                self.bt4.config(text=predicts[7], font=("Courier", 20))
            else:
                self.bt4.config(text="")
